; ============================================================================
; SLIDECPM v0.5.1 - Serial Line Inter-Device (file) Exchange
; Custom file transfer protocol for Z80 / CP/M (wire protocol v0.2.1)
; Target: CP/M 2.2 BIOS console serial path (portable, no direct UART access)
;
...
entry
                ; Save original IOBYTE, then force CON=TTY so serial I/O goes
                ; through standard CP/M BIOS console entry points.
                LD	HL, IOBYTE
                LD	A, (HL)
                LD	DE, iobyte_saved
                LD	(DE), A

                AND	0xFC            ; CON=TTY (bits 1:0 = 00)
                LD	(HL), A
...
; ============================================================================
; CP/M BIOS serial initialisation
; Patch CALL targets for BIOS jump table functions: CONST, CONIN, CONOUT.
; ============================================================================
uart_init
                ; CP/M stores WBOOT jump target at 0x0001/0x0002.
                ; BIOS base = WBOOT - 3; jump table offsets are:
                ;   CONST=+6, CONIN=+9, CONOUT=+12
                LD	HL, (0x0001)      ; HL = WBOOT entry address
                DEC	HL
                DEC	HL
                DEC	HL               ; HL = BIOS base

                LD	DE, 6
                ADD	HL, DE           ; HL = CONST entry
                LD	(bios_const_call + 1), HL

                LD	DE, 3
                ADD	HL, DE           ; HL = CONIN entry
                LD	(bios_conin_call + 1), HL

                ADD	HL, DE           ; HL = CONOUT entry
                LD	(bios_conout_call + 1), HL
...
; ============================================================================
; Serial primitives via CP/M BIOS
; ============================================================================

; Receive a single byte (blocking)
uart_rx
                CALL	bios_const_call
                OR	A
                JR	Z, uart_rx
                CALL	bios_conin_call
                RET
...
; Send a single byte via CP/M BIOS CONOUT
; A = byte to send.  Preserves all registers.
uart_tx
                PUSH	AF
                PUSH	BC
                LD	C, A
                CALL	bios_conout_call
                XOR	A
                LD	(tx_fail), A
                POP	BC
                POP	AF
...
; ============================================================================
; v0.2.2: bit-7-safe protocol byte transmit (see SPEC-v0.2.2.md)
;
; Human-readable text (print_user_msg) is NOT escaped — it goes straight
; through uart_tx as before, since real terminals don't decode this scheme.
; ============================================================================
ESC_HIGH        EQU	0x7E             ; introduces "next byte | 0x80"
ESC_LIT         EQU	0x7D             ; introduces "next byte, taken literally"

; Send a protocol byte, escaping if needed. A = byte to send.
; Preserves all registers (same contract as uart_tx).
proto_tx
                PUSH	AF
                PUSH	BC
                LD	B, A             ; B = byte to send, survives uart_tx calls
...
.lit            ; byte == ESC_HIGH or ESC_LIT — send literally via ESC_LIT
                LD	A, ESC_LIT
                CALL	uart_tx
                LD	A, B
                CALL	uart_tx
...
.high           ; byte >= 0x80 — send as [ESC_HIGH, byte & 0x7F]
                LD	A, ESC_HIGH
                CALL	uart_tx
                LD	A, B
                AND	0x7F
...
; Drain any pending serial input bytes
uart_flush_rx
.flush_loop
                CALL	bios_const_call
                OR	A
...
; Send wakeup signature ESC ^ S L I D E (v0.2.1 §1), then one capability
; byte (v0.2.2 §2): the literal character '7' means "this build escapes
; transmitted protocol bytes per §1 — unescape everything you read from
; me." A deliberately human-readable marker rather than a small binary
; value, so it reads as plain text right after "...SLIDE" in a trace dump
; instead of looking like it could be SOF/CTRL_FIN/etc. Lets one PC client
; auto-detect and interoperate with both this (BIOS-console, bit-7-lossy)
; build and the raw-UART slide.com build, which never sends this byte at
; all — PC clients that don't see it within the handshake window should
; assume the legacy (unescaped) wire format.
; Caller: emit at start of recv_session/send_session, before any RDY
; or framed payload. Lets the PC detect SLIDE.COM is alive.
; None of these bytes (signature or capability flag) ever need escaping
; (all < 0x80, none == ESC_HIGH/ESC_LIT) — routed through proto_tx anyway
; for consistency.
PROTO_CAPS      EQU	'7'              ; "this build escapes tx bytes"
...
.got_fin
                ...
                ; so keep any user-visible text ahead of the final FIN echo.
                ; PC's recv_control already skips
...
.next_file
                ...
                ; print filename - cant do this if we are using CON for IO
                ; LD	DE, msg_sending
                ; CALL	print_user_msg
                ; CALL	print_fcb_name
...
.all_sent
                ...
                ; echo and closes the port, so keep status text before it.
                ; LD	DE, msg_done_session
                ; CALL	print_user_msg
...
compute_file_size
                ...
                ; load into DE, HL
                ; shift all right by one bit and store result one byte higher
                LD	HL, (FCB + 33)
                LD  A, (FCB + 35)

                SRL A                ; logical shift — R2 is unsigned, unlike SRA
                LD  (file_size + 3), A
                RR  H
                RR  L
                LD  (file_size + 1), HL
                LD  A, 0             ; NOT "XOR A" — that would clear carry (R0's
                RRA                  ; LSB, set by the RR L above) before RRA uses it
                LD (file_size), A
...
send_file_tx
                ...
                ; wait for ACK — retry on timeout (v0.2.1: a real ACK reply
                ; can be a lot slower than the ~2s-at-8MHz uart_rx_timeout
                ; window on a fast Z80, so give the PC MAX_RETRIES chances
                ; before giving up, same budget as the data-frame window).
                XOR	A
                LD	(hdr_retry), A
.hdr_wait_ack
                CALL	recv_control_z80
...
.tx_timeout
                ; reserved failure flag; BIOS-backed transmit currently clears it
                LD	A, (tx_fail)
                OR	A
                JR	NZ, .tx_abort
                LD	A, (tx_retry)
...
.frame_loop
                ; reserved failure flag for the transmit path
                LD	A, (tx_fail)
                OR	A
                RET	NZ
...
.sf_payload
                ...
                LD	A, (HL)         ; reload (proto_tx preserves AF but crc trashes A)
                PUSH	HL
                PUSH	BC
                CALL	crc_update_a
                POP	BC
...
.rd_loop
                ...
                LD	BC, 128
                ADD	HL, BC          ; move ptr on
                ADD DE, BC          ; count bytes
...
; Set to 0 to build crc_update_a from the bit-by-bit reference algorithm
; instead of the CRC_TABLE lookup — no call sites need to change either way.
; TEMP: useful for ruling CRC_TABLE corruption in/out of a CRC-mismatch bug;
; flip back to 1 once done, the table version is ~10x faster per byte.
CRC_USE_TABLE   EQU	1

; Update CRC with byte in A
; Trashes: A, HL, DE (table version) / A, HL (bitwise version).  Preserves BC.
crc_update_a
    IF CRC_USE_TABLE
                PUSH	BC
                LD	B, A
                ...
                RET
    ELSE
                ; --- bit-by-bit reference version, no CRC_TABLE dependency ---
                PUSH	BC
                LD	HL, (crc_val)     ; H = crc_val+1 (hi byte), L = crc_val (lo byte)
                XOR	H                 ; crc ^= (byte << 8) — only the hi byte changes
                LD	H, A

                LD	B, 8
.bit_loop
                SLA	L
                RL	H                 ; HL <<= 1 (16-bit); carry = old bit 15 of HL
                JR	NC, .no_xor
                LD	A, H
                XOR	0x10              ; poly 0x1021, high byte
                LD	H, A
                LD	A, L
                XOR	0x21              ; poly 0x1021, low byte
                LD	L, A
.no_xor
                DJNZ	.bit_loop
...
;
; For PC-driven users (original console = TTY), emit bytes via uart_tx so
; status text follows the same serial path as protocol traffic.
;
; For local-console users (original console != TTY), temporarily restore the
; original IOBYTE and route via BDOS so messages appear on their local CON.
; ============================================================================
print_user_msg
                ; CON=TTY ⇒ wire-direct via uart_tx
                LD	A, (iobyte_saved)
